...
//static double prev_frame_delay = 0.0;
    ...
    if (!hardware_decode) is->video_st->codec->flags |= AV_AV_CODEC_FLAG_GRAY;
    // Decode video frame
...
int stream_component_open(VideoState *is, int stream_index)
{
    ...
    {
        if (!hardware_decode) codecCtx->flags |= AV_AV_CODEC_FLAG_GRAY;
        is->dec_ctx = codecCtx;
#ifdef HARDWARE_DECODE
...
//        av_dict_set_int(&opts, "lowres", stream_lowres, 0);
        if (!hardware_decode) {
//            codecCtx->flags |= AV_AV_CODEC_FLAG_GRAY;
