...
//    DUMP_TIMING("a frame", is->audio_clock, apts, base_apts);
    ...
    if (framenum < 2)
        return;
    f = framenum-1;
    while (get_frame_pts(f-1) > base_apts && f > 0) // Find first frame with all samples available
        f--;
    while (f < framenum-1 && get_frame_pts(f+1) <= top_apts && get_frame_pts(f) >= base_apts) {
        volume = retreive_frame_volume(is, get_frame_pts(f), get_frame_pts(f+1));
        set_frame_volume(f, volume);
        f++;
    }
    return;
...
int retreive_frame_volume(VideoState *is, double from_pts, double to_pts)
{
    short *buffer;
    int volume = -1;
    int i;
    int s_per_frame = (to_pts - from_pts) * (double)is->audio_st->codec->sample_rate - 1;
    if (base_apts!= 0 && base_apts <= from_pts && to_pts < top_apts )
    {
        buffer = & audio_buffer[(int)((from_pts - base_apts) * (double)is->audio_st->codec->sample_rate )];
        audio_samples -= (int)(from_pts - base_apts) * (double)is->audio_st->codec->sample_rate; // incomplete frame before complete frame
