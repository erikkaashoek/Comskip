...
int Load_libavcodec()
{
  ...
  // Find the decoder for the video stream
	pCodec = pt_avcodec_find_decoder_by_name("h264");
	if(pCodec==NULL) {
    fprintf(stderr, "Unsupported codec!\n");
    return -1; // Codec not found
  }

	pCodecCtx = pt_avcodec_alloc_context((TlibavcodecExt *) 0 );

	// Open codec
	if(pt_avcodec_open(pCodecCtx, pCodec)<0)
...
//	av_log_set_level(50); 
	...
	pt_avcodec_thread_init(pCodecCtx, 4);
	// Allocate video frame
	pFrame=pt_avcodec_alloc_frame(); 	
...
//			pCodecCtx->lowres=1;
#ifdef DONATORS
			done = pt_avcodec_decode_video(pCodecCtx, pFrame, &frameFinished, data, size); 
			if (done < 0) {
				frameFinished = 0;
			}
#endif
