...
int Load_libavcodec()
{
	...
	DLLLoad( pt_avcodec_find_decoder, "avcodec_find_decoder" );
  ...
  // Find the decoder for the video stream
	pCodec = pt_avcodec_find_decoder(CODEC_ID_H264);
	if(pCodec==NULL) {
...
int h264_decode_frame(unsigned char **frame_ptr, char *data, int size)
{
	avcodec_decode_video(pCodecCtx, pFrame, &frameFinished, data, size); 
	if (frameFinished)
	{
		*frame_ptr = pFrame;
		return(1);
