...
output_format write_format=OF_RAW; // 0=Raw, 1=srt, 2=SMI
...
int __cdecl _CEW_init(int argc, char *argv[])
{
    ...
    for (i=0;i<num_input_files && !processed_enough;i++)
    {
		...
		----------------------------------------------------------------- */
                ...
                case 2:
                    ...
                    else
					{
						...
						{			
							...
							for (unsigned int i=3;i<startbytes_avail;i++)
							{
			...
			if (ts_mode)
			{
				printf ("Min PTS: %u\n",min_pts);
				printf ("Max PTS: %u\n",max_pts);
				int total_secs = (int) ((max_pts-min_pts)/MPEG_CLOCK_FREQ);
		...
		close (in);
        unsigned secs = (int) (totalblockswritten_thisfile()/29.97);
