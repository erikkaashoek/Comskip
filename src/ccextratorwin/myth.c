...
static long mpegps_psm_parse(void)
{
    ...
    myth_fskip(ps_info_length);
    es_map_length = get_be16();

    /* at least one es available? */
    while (es_map_length >= 4)
    {
        unsigned char type = (unsigned char) myth_get_byte();
        unsigned char es_id =(unsigned char) myth_get_byte();
        unsigned int es_info_length = get_be16();
        /* remember mapping from stream id to stream type */
        psm_es_type[es_id] = type;
        /* skip program_stream_info */
        myth_fskip(es_info_length);
...
found:
        ...
        {          
            ...
            if (len <= 3)
                goto skip;
            myth_get_byte(); // emphasis (1), muse(1), reserved(1), frame number(5) 
            myth_get_byte(); // quant (2), freq(2), reserved(1), channels(3) 
            myth_get_byte(); // dynamic range control (0x80 = off) 
            len -= 3;
            //freq = (b1 >> 4) & 3;
