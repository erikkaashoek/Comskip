...
#define RND_INV_ROW (short)(1024 * (6 - BITS_INV_ACC)) //1 << (SHIFT_INV_ROW-1)
#define RND_INV_COL (short)(16 * (BITS_INV_ACC - 3)) // 1 << (SHIFT_INV_COL-1)
#define RND_INV_CORR (short)( RND_INV_COL - 1) // correction -1.0 and round
...
*/
	...
	r0 = _mm_packus_epi16(_mm_load_si128(&src[0]), _mm_load_si128(&src[1]));
	r1 = _mm_packus_epi16(_mm_load_si128(&src[2]), _mm_load_si128(&src[3]));
	r2 = _mm_packus_epi16(_mm_load_si128(&src[4]), _mm_load_si128(&src[5]));
	r3 = _mm_packus_epi16(_mm_load_si128(&src[6]), _mm_load_si128(&src[7]));
...
void mpeg2_idct_add_sse2(const int last, int16_t* block, uint8_t* dest, const int stride)
{
	...
	*/
	...
	r0 = _mm_load_si128(&src[0]);
	r1 = _mm_load_si128(&src[1]);
	r2 = _mm_load_si128(&src[2]);
	r3 = _mm_load_si128(&src[3]);
	r4 = _mm_load_si128(&src[4]);
	r5 = _mm_load_si128(&src[5]);
	r6 = _mm_load_si128(&src[6]);
	r7 = _mm_load_si128(&src[7]);

	q0 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[0*stride]);
	q1 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[1*stride]);
	q2 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[2*stride]);
	q3 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[3*stride]);
	q4 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[4*stride]);
	q5 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[5*stride]);
	q6 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[6*stride]);
	q7 = _mm_loadl_pi(*(__m128*)&zero, (__m64*)&dest[7*stride]);
