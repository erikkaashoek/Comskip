...
//			asf_tag_picture((mtime_t)i_pts + i_payload * (mtime_t)i_pts_delta);
             ...
             i_payload_data_pos += i_sub_payload_data_length )
        {
            ...
            if( tk->p_frame && i_media_object_offset == 0 && 0)
//if (0)
            {
                /* send complete packet to decoder */
                block_t *p_gather = block_ChainGather( tk->p_frame );
printf("\nSend %d bytes to the decoder on es=%d", p_gather->i_buffer,tk->p_es); 
...
// p_gather->i_length
				...
				if (tk->p_es == 1) {
						asf_tag_picture((mtime_t)p_gather->i_pts);
						dump_video_start();
...
if (csFound)
	return(0);
			...
			printf("\nSend %d bytes to buffer on es=%d", p_frag->i_buffer,tk->p_es); 
...
//					dump_audio(&p_peek[i_skip], &p_peek[i_read]);
            ...
            if( tk->p_frame == NULL )
            {
                ...
                /* send complete packet to decoder */
printf("\nSend %d bytes to the decoder on es=%d", p_frag->i_buffer,tk->p_es); 
//                es_out_Send( p_demux->out, tk->p_es, p_gather );
...
//						asf_tag_picture((mtime_t)p_gather->i_pts);
						dump_video_start();
						decode_mpeg2 (p_frag->p_buffer, &p_frag->p_buffer[p_frag->i_buffer]);
...
if (csFound)
	return(0);
					...
					dump_audio_start();
//					set_apts((mtime_t)p_gather->i_pts);
					decode_mpeg2_audio(p_frag->p_buffer, &p_frag->p_buffer[p_frag->i_buffer]);
				}
