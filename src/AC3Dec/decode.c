...
resync:
	...
	if (&cur[2] > end)
		goto done;
//	if (cur[0] != 0x0b || cur[1] != 0x77)	// Check if next frame is again AC3 frame
//		goto done;							// Only works when multiple AC3 frames inside single PS frame
	ret = 1;
done:
...
//	if (ret == -1)
//		return(-1);
	AC3_sampling_rate=syncinfo.sampling_rate;
